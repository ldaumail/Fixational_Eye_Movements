%In this script, we use the code used to select the trials and units with
%clean data from get_clean_peaks_and_data.m to create logical vectors indicating which trials were selected within a selected unit 
...
layer([1,46,55]) = [];
 ...
 %% find peaks and trial index in order to analyze them on R and fit a LMER
...
%make logical vectors to locate the selected trials (1s) and the rejected
%trials (0s) for a selected unit
logicals = struct();
...
clear i
 for i = channum
   ...
   highcontrast = data_file.new_data(i).channel_data.contrast >=  0.5 & data_file.new_data(i).channel_data.fixedc ==  0; 
    ...
    %store logicals for a given unit
    log_trs = highcontrast;
    %get highcontrast trials indices
    log_trs_idx = find(highcontrast);
    stim_trs_idx = data_file.new_data(i).channel_data.trial(log_trs_idx);
    
    %for each trial, baseline correct, filter, compute mean over 280 post
    %stim onset, generate fft
     for tridx = trialidx
  ...
  %%%%%%%%%%% %reject trials below the 95%CI in the blank condition %%%%%%
   ...
   %baseline is an outlier
    ...
    %%% get the elements that are not nans as 1s and the trials that are
    %%% nans as 0s
    log_trs_idx = log_trs_idx(~all(isnan(filtered_dSUA_high)));
    stim_trs_idx = stim_trs_idx(~all(isnan(filtered_dSUA_high)));
   ...
   %MOST IMPORTANT IN THE SCRIPT:
   %logicals(i).idx = log_trs_idx;
   logicals(i).idx = stim_trs_idx;
   logicals(i).penetration = filename;
...
%save(strcat(channelfilename, '.mat'), 'all_pks');
 ...
 %allfilename = ['C:\Users\daumail\Documents\LGN_data\single_units\s_potentials_analysis\analysis\selected_trials_idx'];
 allfilename = ['C:\Users\daumail\Documents\LGN_data\single_units\microsaccades_adaptation_analysis\analysis\stim_selected_trials_idx'];

 save(strcat(allfilename, '.mat'), 'logicals');
 %{
